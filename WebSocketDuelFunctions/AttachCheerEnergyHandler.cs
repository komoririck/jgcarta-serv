...
{
    internal class AttachCheerEnergyHandler
    {
        internal async Task AttachCheerEnergyHandleAsync(DuelAction _DuelAction, MatchRoom cMatchRoom, bool stage, bool collab, bool back, bool TOPCHEERDECK = false, bool FULLCHEERDECK = false, int ClientEnergyIndex = -1, bool ARQUIVEFULLDECK = false, string ENFORCETAG = null, bool FULLTEMPHAND = false)
        {

            _DuelAction.targetCard?.GetCardInfo();
            _DuelAction.usedCard?.GetCardInfo();

            if (!string.IsNullOrEmpty(ENFORCETAG) && !_DuelAction.targetCard.cardTag.Contains(ENFORCETAG)) {
                Lib.WriteConsoleMessage("tag didnt match requirements");
                return;
            }
            ...
            if (ARQUIVEFULLDECK || FULLTEMPHAND)
            {
                List<Card> playerArquiveDeck = new();
                if (ARQUIVEFULLDECK)
                    playerArquiveDeck  = cMatchRoom.currentPlayerTurn == cMatchRoom.playerA.PlayerID ? cMatchRoom.playerAArquive : cMatchRoom.playerBArquive;
                else
                    playerArquiveDeck = cMatchRoom.currentPlayerTurn == cMatchRoom.playerA.PlayerID ? cMatchRoom.playerATempHand : cMatchRoom.playerBTempHand;
                ...
                if (FULLTEMPHAND) { 
                    _DuelAction.usedCard.cardNumber = playerArquiveDeck[n].cardNumber;
                    _DuelAction.usedCard.cardPosition = playerArquiveDeck[n].cardPosition;
                }
            ...
            else
            {
                ...
                if (TOPCHEERDECK)
                {
                    //for cards like hBP01-0941 that the player dont select the cheer
                    if (string.IsNullOrEmpty(SelectedCheerNumber) && playertemphand.Count > 0)
                        SelectedCheerNumber = playertemphand.Last().cardNumber;

                    //if selection didnt match the values we have
